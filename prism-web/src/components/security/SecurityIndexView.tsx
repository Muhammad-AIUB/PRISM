...
import type { SecurityIndexData, SessionUser } from '@/lib/types';
import {
    AlertCircle,
    Check,
    Database,
...
function YesRow({ children }: { children: ReactNode }) {
    return (
...
function NoRow({ children }: { children: ReactNode }) {
    return (
        <li className="flex items-start gap-3">
            <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full"
...
function TimelineStep({ icon, title }: { icon: ReactNode; title: string }) {
    return (
        <li className="flex items-start gap-3">
...
function FlowStep({ step, title, sub }: { step: number | string; title: string; sub: string }) {
    return (
...
function GuestSecurityLayout({ children }: { children: ReactNode }) {
    return (
...
/**
 * Public by design: visitors read this BEFORE authorising anything, which is
 * exactly when it matters. Signed-in users get the app chrome around it;
 * everyone else gets a standalone header with a sign-in button.
 */
export default function SecurityIndexView({
    data,
    user,
}: {
    ...
    const { github_app_url, github_repo_url, is_authenticated } = data;
...
localStorage.setItem('prism-theme', theme);   // 'light' | 'dark'`}
    ...
    return <GuestSecurityLayout>{body}</GuestSecurityLayout>;
}
